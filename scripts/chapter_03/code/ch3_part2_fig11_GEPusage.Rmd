...
plot_beeswarm <- function(seurobj=seur_integrated, feature, xaxis="tissue", ytitle, cols_vector=cols_tissue, file_name="no"){
  seurobj@meta.data$tissue <- factor(seurobj@meta.data$tissue, levels=c("Thymus", "PBMC"))
  p <- ggplot(
    as.data.frame(seurobj@meta.data),
    aes_string(x=xaxis, y=feature, fill=xaxis)
  ) +
...
## Plot all GEPs usage on beeswarmplots
```{r plot-gep-usage-beeswarm, fig.width=14, fig.height=10}
plist_beeswarm <- list()
for(i in 1:11){
  print(i)
